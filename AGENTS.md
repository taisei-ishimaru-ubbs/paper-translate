...
- `origin` は `taisei-ishimaru-ubbs/paper-translate` fork、`upstream` は `taiseee/paper-translate` 原本とする。upstream へ直接 push しない。
- スクリプト・設定・`gallery.md`・`.obsidian` の共有設定に加え、`papers/` の原文・翻訳・要約・引用・図・ノート・symlink を commit 対象とする。
